...
publish(RoutingKey, Msg)->
    QueueMonitorEnabled =
      oc_chef_action_queue:get_rabbit_queue_monitor_setting(queue_length_monitor_enabled, false),
...
publish(RoutingKey, Msg, true) ->
    DropOnCapacity =
      oc_chef_action_queue:get_rabbit_queue_monitor_setting(drop_on_full_capacity, true),
...
ping() ->
    ...
    case chef_wm_rabbitmq_management:check_aliveness(
           oc_chef_action_queue:get_rabbit_management_pool_name(), binary_to_list(VHost)) of
        true -> pong;
        _ -> pang
    end.
