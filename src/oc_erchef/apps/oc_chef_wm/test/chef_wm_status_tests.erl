    ...
    {setup,
     ...
     [
      fun() ->
              ...
              oc_chef_action_queue:set_rabbit_queue_monitor_setting(queue_at_capacity_affects_overall_status, true),
              {Status, Json} = chef_wm_status:check_health(),
              ?assertEqual(fail, Status),
              Ejson = chef_json:decode(Json),
      ...
      fun() ->
              ...
              oc_chef_action_queue:set_rabbit_queue_monitor_setting(queue_at_capacity_affects_overall_status, false),
              {Status, Json} = chef_wm_status:check_health(),
              ?assertEqual(pong, Status),
              Ejson = chef_json:decode(Json),
              ?assertEqual(<<"pong">>, ej:get({<<"status">>}, Ejson)),
